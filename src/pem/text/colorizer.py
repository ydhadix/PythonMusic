"""Syntax colouring for the editor's Text widget.

A ``ColorDelegator`` sits in the editor's percolator chain and tags keywords,
strings, comments, builtins, definitions, etc. as the text changes.  Recolouring
runs incrementally on the idle loop so large files don't stall typing.
"""
import builtins
import keyword
import re
...
def color_config(text):
    ...
    normal_colors = pemConf.GetHighlight(theme, 'normal')
    cursor_color = pemConf.GetHighlight(theme, 'cursor')['foreground']
    select_colors = pemConf.GetHighlight(theme, 'hilite')
    text.config(
...
class ColorDelegator(Delegator):
    ...
    def LoadTagDefs(self):
        ...
        theme = pemConf.CurrentTheme()
        self.tagdefs = {
            "COMMENT": pemConf.GetHighlight(theme, "comment"),
            "KEYWORD": pemConf.GetHighlight(theme, "keyword"),
            "BUILTIN": pemConf.GetHighlight(theme, "builtin"),
            "STRING": pemConf.GetHighlight(theme, "string"),
            "DEFINITION": pemConf.GetHighlight(theme, "definition"),
            "SYNC": {'background': None, 'foreground': None},
            "TODO": {'background': None, 'foreground': None},
            "ERROR": pemConf.GetHighlight(theme, "error"),
            ...
            "hit": pemConf.GetHighlight(theme, "hit"),
            }
...
def _color_delegator(parent):  # htest #
    ...
    from pem.pem_test.test_colorizer import source
...
if __name__ == "__main__":
    ...
    main('pem.pem_test.test_colorizer', verbosity=2, exit=False)
