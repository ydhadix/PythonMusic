...
class FormatParagraph:
    ...
    * Fancy comments, like this bulleted list, aren't handled :-)
    ...
    @classmethod
    def reload(cls):
        cls.max_width = pemConf.GetOption('extensions', 'FormatParagraph',
    ...
    def format_paragraph_event(self, event, limit=None):
        """Formats paragraph to a max width specified in pemConf.
...
class FormatRegion:
    ...
    def toggle_comment_event(self, event=None):
        """Toggle comments for each line in region.

        If all lines are commented (start with ## or #), uncomment them.
        Otherwise, comment all lines with a single #.
        """
        head, tail, chars, lines = self.get_region()
        if not lines:
            return "break"
        ...
        all_commented = True
        for line in lines:
            if line and not (line[:2] == '##' or line[:1] == '#'):
                all_commented = False
                break
...
class Rstrip:  # "Strip Trailing Whitespace" on the Format menu.
    "Removes trailing whitespace from every line of the editor buffer."
...
if __name__ == "__main__":
    ...
    main('pem.pem_test.test_format', verbosity=2, exit=False)
