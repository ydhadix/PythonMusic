...
class _SimpleBinder:
    "Binder for event types with no modifiers and no detail (e.g. <<paste>>)."
...
class _ComplexBinder:
    "Binder for event types with modifiers and/or a detail (e.g. <Key-a>, <Control-Button-1>)."
    ...
    def __del__(self):
        # -------------------------------------------------------------
        # SHUTDOWN FAILSAFE
        # When the application closes, the widget may be destroyed before 
        # the garbage collector runs. Silently catch and ignore all exceptions 
        # to prevent noisy terminal traces on exit.
        # -------------------------------------------------------------
                ...
                if func:
                    for triplet in triplets:
                        try:
                            ...
                            # SHUTDOWN FAILSAFE: Ignore teardown errors
                            pass
...
if __name__ == "__main__":
    ...
    main('pem.pem_test.test_mainmenu', verbosity=2, exit=False)
