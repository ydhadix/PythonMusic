...
class SearchDialogBase:
    ...
    def _install_focus_tracker(self):
        # The dialog is modeless, so the user can click into a different
        # editor while it's open. Retarget self.text on FocusIn so the
        # command buttons act on whichever editor has focus, matching
        # what the keyboard shortcut already does. Filter by the <<find>>
        # binding to ignore non-editor Text widgets (e.g. the shell).
        def on_text_focus(event):
            if event.widget.bind("<<find>>"):
                self.text = event.widget
        self.root.bind_class("Text", "<FocusIn>", on_text_focus, add="+")
...
class _searchbase(SearchDialogBase):  # htest #
    ...
    def __init__(self, parent):
        import re
        from pem.searching import searchengine
...
if __name__ == '__main__':
    ...
    main('pem.pem_test.test_searchbase', verbosity=2, exit=False)
