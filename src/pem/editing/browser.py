...
from pem.editing.tree import TreeNode, TreeItem, ScrolledCanvas
from pem.util import py_extensions
from pem.window import ListedToplevel
...
class ModuleBrowser:
    """Browse module classes and functions in PEM.
    """
    ...
    def init(self):
        ...
        background = pemConf.GetHighlight(theme, 'normal')['background']
...
if __name__ == "__main__":
    if len(sys.argv) == 1:  # If pass file on command line, unittest fails.
        ...
        main('pem.pem_test.test_browser', verbosity=2, exit=False)
