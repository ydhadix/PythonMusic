...
of each PEM edit window which provides block structure hints.  These hints are
...
class CodeContext:
    ...
    def reload(cls):
        ...
        cls.context_depth = pemConf.GetOption("extensions", "CodeContext",
                                               "maxlines", type="int",
                                               default=15)
    ...
    def toggle_code_context_event(self, event=None):
        ...
        if self.context is None:
            ...
            line_number_colors = pemConf.GetHighlight(pemConf.CurrentTheme(),
                                                       'linenumber')
            ...
            self._reset()
            menu_status = 'Show'
        # Update menu label if options menu exists (it's commented out now)
        try:
            self.editwin.update_menu_label(menu='options', index='*ode*ontext',
                                           label=f'{menu_status} Code Context')
        except KeyError:
            pass   # options menu doesn't exist
        return "break"
    ...
    def update_highlight_colors(self):
        if self.context is not None:
            colors = pemConf.GetHighlight(pemConf.CurrentTheme(), 'context')
...
if __name__ == "__main__":
    ...
    main('pem.pem_test.test_codecontext', verbosity=2, exit=False)
