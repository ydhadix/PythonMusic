...
class PianoRoll:
   ...
   def __init__(self, score, quantum=0.25, maxVoiceJump=7):
      ...
      # quantize notes in-place by "snapping" to nearest gridline
      # (moving start and end of note to nearest multiple of quantum)
      self.noteArray = self._quantizeNotes(self.noteArray, self.quantum)
      ...
      # populate the piano roll with the note array indices
      ...
      for part in parts:
         ...
         for phrase in phrases:
            ...
            for n in range(phrase.getSize()):
               ...
               currentTime += note.getDuration()   # update the current time to note's end time (duration = notated time)
   ...
   def _snapToNearestGridline(self, timeInQuantums):
      """
      Rounds to nearest multiple of quantum, with exact midpoints rounding down.

      param float timeInQuantums: Time value divided by quantum (e.g., 0.3 / 0.5 = 0.6)
      return int: Nearest integer, with .5 values rounding down
      """
      fractionalPart = timeInQuantums - math.floor(timeInQuantums)

      if fractionalPart < 0.5:
         return math.floor(timeInQuantums)   # closer to earlier boundary
      elif fractionalPart > 0.5:
         return math.ceil(timeInQuantums)   # closer to later boundary
      else:   # exactly at midpoint
         return math.floor(timeInQuantums)   # midpoint snaps down

   def _quantizeNotes(self, notes, quantum):
      """
      Quantizes notes by snapping start and end times to nearest gridline.
      Modifies notes in-place and drops notes that collapse to same boundary.

      param list[ExtendedNote] notes: The note array (index 0 is empty)
      param float quantum: The quantum duration for quantization
      return list[ExtendedNote]: The same array with notes quantized (some may be removed)
      """
      quantizedNotes = [None]   # index 0 remains empty

      for noteIndex in range(1, len(notes)):
         note = notes[noteIndex]

         if note is None:
            continue

         originalStart = note.getStartTime()
         originalDuration = note.getDuration()
         originalEnd = originalStart + originalDuration

         # snap both start and end to nearest quantum boundaries
         startInQuantums = originalStart / quantum
         endInQuantums = originalEnd / quantum

         snappedStartQuantums = self._snapToNearestGridline(startInQuantums)
         snappedEndQuantums = self._snapToNearestGridline(endInQuantums)

         snappedStart = snappedStartQuantums * quantum
         snappedEnd = snappedEndQuantums * quantum
         snappedDuration = snappedEnd - snappedStart

         # drop notes that collapse to same quantum boundary
         if snappedDuration <= 0:
            continue

         # update the note's start time and duration to snapped values
         note.setStartTime(snappedStart)
         note.setDuration(snappedDuration)

         # also set the quantized values (will be refined in _populatePianoRoll)
         note.setStartTimeQuantized(snappedStart)
         note.setDurationQuantized(snappedDuration)

         quantizedNotes.append(note)
   ...
   def _populatePianoRoll(self, notes, quantum, scoreEndTime):
      ...
      for timeSlice in range(rasterLength):
         ...
         while noteIndex < len(notes) and notes[noteIndex].getStartTime() < currentTime + quantum:
            ...
            if notes[noteIndex].getDuration() >= quantum:   # duration = notated time
               ...
               noteEndTime = currentTime + notes[noteIndex].getDuration()   # duration = notated time
   ...
   def getNote(self, index):
      ...
      return ExtendedNote: the corresponding note from the Score.
      """
   ...
   def getNextNoteSameDuration(self, duration, timeIndex):
      ...
      while timeIndex < self.getLength() and not found:   # while next note is not found and there are more time slices left
         ...
         while index < len(timeSlice) and not found:   # while next note is not found and still notes within this time slice
            if timeSlice[index] > 0:   # if a new note found
               nextNote = self.getNote(timeSlice[index])   # check this note
               if duration == nextNote.getDuration():   # duration = notated time
                  found = True   # ...we are done!
            index += 1   # point to another note (if any)
         # postcondition: we either found the note, or this time slice is finished
         ...
         for timeSliceIndex in range(len(timeSlice)):
            ...
            if timeSlice[timeSliceIndex] > 0:
               ...
               if prevNote is not None:   # if there is a previous note...
                  ...
                  prevNoteEndTime = prevNote.getStartTime() + prevNote.getDuration()   # duration = notated time
