...
class ContourBasslineDurationQuantizedMetric(Metric):
   """
   It implements a Zipf metric capturing quantized duration proportion of the lowest voice (bassline contour) in a music piece.
   ...
   def __init__(self, numHigherOrder=0, level=0):
      """
      It constructs a ContourBasslineDurationQuantizedMetric object for rank-frequency distribution.
      ...
      """
      super().__init__("ContourBasslineDurationQuantized", ZipfMetrics.SIMPLE, ZipfMetrics.BY_RANK, level, numHigherOrder)

   def cloneHigherOrder(self):
      """
      It returns a higher-order instance of this Metric class.
      """
      return ContourBasslineDurationQuantizedMetric(self.numHigherOrder, self.level + 1)

   def measureTimeSlice(self, pianoRoll, timeIndex):
      """
      It counts the quantized duration of the lowest pitch found in the specified time slice of the piano roll,
