...
class ContourMelodyDurationQuantizedMetric(Metric):
   """
   It implements a Zipf metric capturing duration proportion of the highest voice (melody contour) in a music piece.
   ...
   def __init__(self, numHigherOrder=0, level=0):
      """
      It constructs a ContourMelodyDurationQuantizedMetric object for rank-frequency distribution.
      ...
      """
      super().__init__("ContourMelodyDurationQuantized", ZipfMetrics.SIMPLE, ZipfMetrics.BY_RANK, level, numHigherOrder)

   def cloneHigherOrder(self):
      """
      It returns a higher-order instance of this Metric class.
      """
      return ContourMelodyDurationQuantizedMetric(self.numHigherOrder, self.level + 1)

   def measureTimeSlice(self, pianoRoll, timeIndex):
      """
      It counts the quantized duration of the highest pitch found in the specified time slice of the piano roll,
      ...
      if len(timeSlice) > 0:   # is there a note?
         if timeSlice[-1] > 0:   # is this a new note (as opposed to a continuation of an earlier note)
            ...
            quantizedDuration = note.getDurationQuantized()     # get this note's duration
            self.count(float(quantizedDuration))                # capture the change in proportion introduced by this quantized duration
