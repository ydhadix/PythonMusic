...
class ContourBasslinePitchMetric(Metric):
   """
   It implements a Zipf metric capturing pitch proportion of the lowest voice (bassline contour) in a music piece.
   ...
   def __init__(self, numHigherOrder=0, level=0):
      """
      It constructs a ContourBasslinePitchMetric object for rank-frequency distribution.
      ...
      """
      super().__init__("ContourBasslinePitch", ZipfMetrics.SIMPLE, ZipfMetrics.BY_RANK, level, numHigherOrder)

   def cloneHigherOrder(self):
      """
      It returns a higher-order instance of this Metric class.
      """
      return ContourBasslinePitchMetric(self.numHigherOrder, self.level + 1)

   def measureTimeSlice(self, pianoRoll, timeIndex):
      """
      It counts the lowest pitch found in the specified time slice of the piano roll,
